...
import numpy as np
import os
from constants import MODEL_SEG_N, MODEL_SEG_S, MODEL_SEG_M, MODEL_SEG_L, MODEL_SEG_X


def get_prediction(model_path, img_path, output_dir, type_model_load):
    model_seg = ''
    if type_model_load == 'n':
        model_seg = MODEL_SEG_N
    elif type_model_load == 's':
        model_seg = MODEL_SEG_S
    elif type_model_load == 'm':
        model_seg = MODEL_SEG_M
    elif type_model_load == 'l':
        model_seg = MODEL_SEG_L
    elif type_model_load == 'x':
        model_seg = MODEL_SEG_X

    model = YOLO(model_seg)  # load an official model
    model = YOLO(model_path)

    img = cv2.imread(img_path)

    results = model.predict(img, conf=0.05, retina_masks=True,  iou=0.25,
                            agnostic_nms=True)
...
def postprocess_prediction(img, prediction):
    ...
    for mask_point in all_mask_points:
        ...
        text_position = (
            (mask_point["points"][0][0] +
             mask_point["points"][2][0] - text_size[0])
            // 2,
            (mask_point["points"][0][1] +
             mask_point["points"][2][1] + text_size[1])
            // 2,
        )
        cv2.putText(
            result, text, text_position, font, font_scale, (
...
if __name__ == "__main__":
    ...
    count = get_prediction(args.model, args.img,
                           output_dir, args.type_model_load)
